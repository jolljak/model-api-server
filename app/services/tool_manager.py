...
class DynamicToolManager:
                    ...
                    try:
                        async with httpx.AsyncClient(timeout=30.0) as client:
                            ...
                            if resp.status_code < 300:
                                ...
                                print(f"[DEBUG] Response Error: {resp.status_code} - {resp.text}")
    ...
    async def register_preset(self, request: ServicePresetRequest) -> ServiceDefinition:
        ...
        cfg = request.config.copy()
        
        # [Fix] Config Key Normalization
        # 1. Remove namespacing (e.g., "google_calendar.runtimeAuthKey" -> "runtimeAuthKey")
        for k, v in list(cfg.items()):
            if "." in k:
                short_key = k.split(".")[-1]
                if short_key not in cfg:
                    cfg[short_key] = v
        
        # 2. Map legacy keys to standard keys
        if "token" not in cfg and "runtimeAuthKey" in cfg:
            cfg["token"] = cfg["runtimeAuthKey"]
        if "token" not in cfg and "apiToken" in cfg:
            cfg["token"] = cfg["apiToken"]
        ...
        # 2. Notion
            ...
            token = cfg.get("token")
            # 사용자가 설정에서 속성명을 지정할 수 있도록 함 (기본값은 '제목' 등 케이스 구분 필수)
            # title_property -> title_column -> name_property -> name_column 순서로 확인
            title_prop = cfg.get("title_property", cfg.get("title_column", cfg.get("name_property", cfg.get("name_column", "제목"))))
            ...
            definition = ServiceDefinition(
                ...
                payloadTemplate=json.dumps({
                    ...
                    "properties": {
                        title_prop: { "title": [ { "text": { "content": "{meeting_title}" } } ] }
                    },
                    "children": [
                        {
                            "object": "block",
        ...
        # 3. Jira
            ...
            # Jira basic auth (email:apiToken) encoding
            ...
            definition = ServiceDefinition(
                ...
                apiUrlTemplate=f"https://{domain}.atlassian.net/rest/api/2/issue",
                authType="basic",
                methodList="POST",
                payloadTemplate=json.dumps({
                    "fields": {
